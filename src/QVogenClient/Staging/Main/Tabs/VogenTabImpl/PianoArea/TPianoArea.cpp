...
TPianoArea::TPianoArea(QWidget *parent) : QWidget(parent), m_currentHeight(30) {
    init();
}

TPianoArea::TPianoArea(int currentHeight, QWidget *parent)
    : QWidget(parent), m_currentHeight(currentHeight) {
    init();
}
...
void TPianoArea::handleUpdateToneLabel() {
    ...
    QString name = ToneNumToToneName(m_currentNoteNum);
    if (name.contains('#')) {
